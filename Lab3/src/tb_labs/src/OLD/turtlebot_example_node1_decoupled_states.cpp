...
//#define SIMULATION //comment out when running in on turtlebot
...
const double Q = 0.1;                 //noise for x and y, assume decoupled
const double Qt = 0.1*M_PI / 180;     //noise for theta, assume decoupled
const double R = 0.01;                //disturbance variance in x
const double Rt = 0.1 * M_PI/ 180;   //theta disturbance variance
...
void init()
{
    ...
    // has the benefit of not needing transforms for odom
    odom_x = ips_x;
    odom_y = ips_y;
    odom_yaw = ips_yaw;
    ...
    std::normal_distribution<double> particle_v_scatter(0, 5*R); //5 sigma about mean
    std::normal_distribution<double> particle_w_scatter(0, 5*Rt);

    // std::normal_distribution<double> particle_scatter(0, 2); //REALLY BIG
    // std::normal_distribution<double> particle_angle_scatter(0, 5*Qt);    
    // std::normal_distribution<double> particle_v_scatter(0, 2); //5 sigma about mean
    // std::normal_distribution<double> particle_w_scatter(0, 5*Rt);
    ...
    for (int i = 0; i < N_particles; i++)
    {
        ...
        // particles[i].vx = odom_vx + particle_v_scatter(generator);
        // particles[i].vy = odom_vy +  particle_v_scatter(generator);
        // particles[i].w = odom_w +  particle_v_scatter(generator);
...
{
    static std::normal_distribution<double> xy_dither(0, Q);
    static std::normal_distribution<double> angle_dither(0, Qt);
    const double scaling = 2.17;
    
    int i;
    for(i = 0; i < msg.name.size(); i++) if(msg.name[i] == "mobile_base") break;

    true_x = scaling*msg.pose[i].position.x;
    true_y = scaling*msg.pose[i].position.y;
    true_yaw = tf::getYaw(msg.pose[i].orientation);

    double x_dith = xy_dither(generator);
    double y_dith = xy_dither(generator);

//    ROS_DEBUG("x dither: %f\n y dither %f\n", x_dith, y_dith);

    ips_x = true_x + x_dith;
    ips_y = true_y + y_dith;
    ips_yaw = true_yaw; //Not required to mess with yaw :)
...
void pose_callback(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
    ...
    true_x = msg.pose.pose.position.x; // Robot X psotition
    true_y = msg.pose.pose.position.y; // Robot Y psotition
...
void odom_callback(const nav_msgs::Odometry& msg)
{
    ...
    odom_vx = msg.twist.twist.linear.x * cos(odom_yaw);
    odom_vy = msg.twist.twist.linear.x * sin(odom_yaw);
    ...
    odom_x = odom_x + odom_vx * dt;
    odom_y = odom_y + odom_vy * dt;
...
int main(int argc, char **argv)
{
    ...
    ips_points.header.frame_id = "/my_frame";
    ips_points.id = 0;
    ...
    odom_points.header.frame_id = "/my_frame";
    odom_points.id = 1;
    ...
    belief_points.header.frame_id = "/my_frame";
    belief_points.id = 2;
    ...
    belief_line.header.frame_id = "/my_frame";
    belief_line.id = 3;
    ...
    state_points.header.frame_id = "/my_frame";
    state_points.id = 4;
    ...
    true_path_line.header.frame_id = "/my_frame";
    true_path_line.id = 5;
    ...
    while (ros::ok())
    {
        ...
        for (int i = 0; i < N_particles; i++)
        {
         // MATLAB for this part
         // for i=1:I
         //     ep = RE*sqrt(Re)*randn(n,1);
         //     Xp(:,i) = Ad*X(:,i) + Bd*u(:,i) + ep;
         //     w(i) = max(0.00001, normpdf(y(:,t), sqrt(Xp(1,i)^2 + Xp(3,i)^2),sqrt(Q)));
         // end

            particles[i].x = particles[i].x + odom_vx * dt  + xy_disturbances(generator);
            particles[i].y = particles[i].y + odom_vy * dt  + xy_disturbances(generator);
        ...
        if (new_ips) //Only re-sample when new pos data available
        {
            ...
            for (int i = 0; i < N_particles; i++)
            {
                ...
                // particles[i].weight = 0; //reset weight
                // for (int j = 0; j < n_states; j++)
                // {
                //     //ROS_DEBUG("pdfs[%d] = %f", j, pdfs[j]);
                //     particles[i].weight += pdfs[j];  //just add up weights

                //     //Weight sensor measurements by 1/sigma
                //     //Means that most accurate measurements
                //     //weighted higher
                //     //DOES NOT WORK!! -> b/c units are different,
                //     //angles are weighted waaaay too highly       
                //     // particles[i].weight += pdfs[j]/ (100 * sigmas[j]); 

                //     //get highest weight for this particle (doesn't work)
                //     // if (pdfs[j] > particles[i].weight)
                //     // {
                //     //     particles[i].weight = pdfs[j]; 
                //     // }
                // }
